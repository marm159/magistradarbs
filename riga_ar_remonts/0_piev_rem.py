...
# Pārvērš kolonas 'Remonts' vērtības par int, ja ir tukša vērtība, ievieto 'NULL'
...
df_rem['Gads'] = df_rem['Darījuma datums'].apply(lambda x: datetime.strptime(x, "%d.%m.%Y").year if pd.notna(x) else "")
df_rem['Mēnesis'] = df_rem['Darījuma datums'].apply(lambda x: datetime.strptime(x, "%d.%m.%Y").month if pd.notna(x) else "")

# Mēnesim piešķir sezonu
sezonas = []
for mēnesis in df_rem['Mēnesis']:
