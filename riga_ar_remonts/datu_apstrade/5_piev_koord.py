...
file1 = "riga_ar_remonts/datu_apstrade/2024_adr.csv"
...
file2 = "riga_ar_remonts/datu_apstrade/2024_fin.csv"

df = pd.read_csv(file1, sep=';')
df_k = pd.read_csv(file_koord)

# Izveido vārdnīcu meklēšanai pēc jaunajām adresēm: (iela, numurs) → (platums, garums)
jauna_adr = {
    (row["jauna_iela"], row["jauns_nr"]): (row["Platums"], row["Garums"])
    for _, row in df_k.iterrows()
}

# Izveido vēl vienu vārdnīcu meklēšanai pēc vecajām adresēm: (iela, numurs) → (platums, garums)
veca_adr = {
    (row["veca_iela"], row["vecs_nr"]): (row["Platums"], row["Garums"])
    for _, row in df_k.iterrows()
}

# Tukši saraksti, kuros glabās platumu un garumu vērtības
platumi = []
garumi = []

# Iet cauri katrai datu rindai darījumu failā
for ind, row in df.iterrows():
    iela = str(row["Iela"]).strip()  # Iegūst ielas nosaukumu un noņem liekās atstarpes
    majas_numurs = str(row["Mājas numurs"]).strip()  # Iegūst mājas numuru un noņem liekās atstarpes
...
df = df.drop(['Adreses pieraksts', 'Iela', 'Mājas numurs'], axis=1)
