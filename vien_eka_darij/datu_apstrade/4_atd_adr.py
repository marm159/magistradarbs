...
file1 = 'vien_eka_darij/datu_apstrade/2024_fil.csv'
# Izejas fails, kas saturēs atsevišķas kolonas ar ielas nosaukumu un mājas numuru
file2 = 'vien_eka_darij/datu_apstrade/2024_adr.csv'

df = pd.read_csv(file1, sep=';')

# Tukši saraksti, kuros glabās ielu nosaukumus un mājas numurus
ielas = []
majas_nrs = []

# Ielu nosaukumi, kas satur skaitļus tiek atsevišķi apstrādāti
...
for adrese in df['Adreses pieraksts']:
    ...
    # Pārbauda, vai adrese satur 13. janvāra iela vai 11. novembra krastmala
    for iela_sk in ielas_ar_sk:
        if iela_sk in adrese:
            iela = iela_sk  # Piešķir 13. janvāra iela vai 11. novembra krastmala
            pareja_adr = adrese[len(iela_sk):].strip().split()  # Pārējā adreses daļu pēc ielas sadala pa atstarpēm

            # Meklē mājas nummuru
            for i, dala in enumerate(pareja_adr):
                if dala[0].isdigit(): # pārbauda, vai konkrētā daļa sākas ar ciparu (jo mājas numurs sākas ar ciparu).
                    ...
                    for j in range(i + 1, len(pareja_adr)): # Turpina meklēt, vai mājas numuram nav vēl kāda daļa, piemēram, 1A vai k-11
                        ...
                        majas_nr += " " + pareja_adr[j] # Ja nav sastapts "-", tad pievieno attiecīgo daļu mājas numuram
                    break
            break
    ...
    if not iela:
        ...
        if "līnija" in adrese:
            for i, dala in enumerate(adrese_dalas): # Iet cauri katrai adreses daļai
                if "līnija" in dala: # Kad atrod vārdu "līnija", 
                    iela = " ".join(adrese_dalas[:i + 1])  # ielas nosaukums ir viss līdz "līnija" ieskaitot
                # Ja atrod pirmo skaitli pēc ielas, un pirms tā ir vārds "līnija", tad tas is mājas numurs
                    ...
                    for j in range(i + 1, len(adrese_dalas)): # Turpina meklēt, vai mājas numuram nav vēl kāda daļa, piemēram, 1A vai k-11
                        if adrese_dalas[j] == "-":  # Ja tiek atrasts "-"
                            break # iziet no šī cikla, jo vairs nav jāmeklē tālāk.
                        majas_nr += " " + adrese_dalas[j] # Ja nav sastapts "-", tad pievieno attiecīgo daļu mājas numuram
                    break
        else:
    ...
    # Saglabā iegūto ielu un mājas numuru ielas un mājas numuru sarakstos
    ielas.append(iela)
