...
file1 = 'vien_eka_darij/datu_apstrade/2024.csv'
# Izejas fails, kas saturēs datus bez trokšņiem
file2 = 'vien_eka_darij/datu_apstrade/2024_fil.csv'
# Izejas fails, kas saturēs trokšņus
file3 = 'vien_eka_darij/datu_apstrade/2024_troksni.csv'
...
# Meklē trokņus datiem, kur ēka ekspluatēta pirms 2000. gada
Q1_pirms = pirms_2000['EUR/m^2'].quantile(0.25)  # 1. kvartile (25%)
Q3_pirms = pirms_2000['EUR/m^2'].quantile(0.75)  # 3. kvartile (75%)
...
# Meklē trokņus datiem, kur ēka ekspluatēta pēc 2000. gada
Q1_pec = pec_2000['EUR/m^2'].quantile(0.25)  # 1. kvartile
Q3_pec = pec_2000['EUR/m^2'].quantile(0.75)  # 3. kvartile
IQR_pec = Q3_pec - Q1_pec                    # Starpkvartilu attālums
...
# Apvieno abus trokšnu datus
outliers = pd.concat([outliers_pirms_2000, outliers_pec_2000])

# No sākotnējās datu kopas tiek izņemtas tās datu rindas, kuru indeksi sakrīt ar indeksiem trokšnu datu kopā
df = df[~df.index.isin(outliers.index)]

# Izdzēš kolonnu 'EUR/m^2', jo tā vairs nav nepieciešama
df = df.drop(['EUR/m^2'], axis=1)
